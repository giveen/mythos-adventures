...
router.delete('/model/:name', (req, res) => {
  ...
  try {
    console.log('[ollamaRoutes2] DELETE /model/:name received:', name);
    const result = ollama.deleteModel(name);
    console.log('[ollamaRoutes2] delete result:', result);
    res.json(result);
  } catch (e) {
...
router.post('/model/delete', (req, res) => {
  ...
  try {
    console.log('[ollamaRoutes2] POST /model/delete received:', name);
    const result = ollama.deleteModel(name);
    console.log('[ollamaRoutes2] delete result (body):', result);
    res.json(result);
  } catch (e) {
    res.status(500).json({ error: String(e && e.message) });
  }
